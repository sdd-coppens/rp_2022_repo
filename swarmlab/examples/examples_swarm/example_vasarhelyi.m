function score_scaled = example_vasarhelyi(random_scale, random_scale_scale, nb_agents)
    ...
    if exist('app', 'var')
        ...
        % Swarming parameters
        algo_change = app.swarming_defect;
        front_sensor_failure_chance = app.front_sensor_failure_chance;
        front_sensor_failure_scale = app.front_sensor_failure_scale;
        ...
        % If not on:
        algo_change = 'no_front_random_and_scale';
        %algo_change = 'no_front_random';
        front_sensor_failure_chance = random_scale;
        front_sensor_failure_scale = random_scale_scale;
        DEBUG = false;
        VIDEO = false;
    ...
    swarm.swarm_defect = algo_change;
    swarm.front_sensor_failure_chance = front_sensor_failure_chance;
    swarm.front_sensor_failure_scale = front_sensor_failure_scale;
    ...
    nb_collisions_obstacle = 0;
    nb_collisions_agents = 0;
    ...
    for time = p_sim.start_time:p_sim.dt:p_sim.end_time
        ...
        % Sum all collisions
        nb_collisions_obstacle = nb_collisions_obstacle + collisions(2);
        nb_collisions_agents = nb_collisions_agents + collisions(1);
    ...
    % Calculate score of run. Following weights:
    % collision_percentage_obstacle :     0.3
    % collision_percentage_agents :       0.3
    % average_order_swarm :               0.2
    % average_alg_conn_swarm :            0.2
    %
    % This score is then formed according to y = x^4.3.
    %
    % To punish collisions more their percentages shall be scaled by an amount of
    scalar_collision = 10;
    collision_percentage_obstacle = min(1 - (scalar_collision * (nb_collisions_obstacle / length(p_sim.start_time:p_sim.dt:p_sim.end_time))), 1);
    collision_percentage_agents = min(1 - (scalar_collision * (nb_collisions_agents / length(p_sim.start_time:p_sim.dt:p_sim.end_time))), 1);
    
    pos_ned_history = swarm.get_pos_ned_history();
    pos_ned_history = pos_ned_history(2:end,:);
    vel_ned_history = swarm.get_vel_xyz_history();
    [safety, order, union, alg_conn, safety_obs, min_d_obs] = ...
        compute_swarm_performance(pos_ned_history, vel_ned_history, p_swarm, results_dirname);
    
    mean_alg_con = mean(alg_conn);
    mean_order = mean(order);
    if ~isreal(mean_alg_con)
        mean_alg_con= 0;
    end
    if ~isreal(mean_order)
        mean_order = 0;
    end

    weighted_score_run = (0.3 * collision_percentage_obstacle + 0.3 * collision_percentage_agents + 0.2 * mean_order + 0.2 * mean_alg_con);
    score_scaled = weighted_score_run^4.3;
